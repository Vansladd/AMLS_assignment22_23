...
img_files = os.listdir('datasets\cartoon_set\img')
print(len(img_files))
df = pd.read_csv("datasets\cartoon_set\labels.csv")
df.columns = ['a']
...
checkpointer = ModelCheckpoint('modelB2.h5', monitor='val_loss', verbose=1, save_best_only=True)

# train the model

B2 = model.fit_generator(train_generator,
                    steps_per_epoch=train_generator.n // train_generator.batch_size,
                    epochs=8 ,
                    validation_data=validation_generator,
                    validation_steps=validation_generator.n // validation_generator.batch_size,
                    callbacks=[earlystopper, checkpointer]) 
acc = B2.history['accuracy']
val_acc = B2.history['val_accuracy']
loss = B2.history['loss']
val_loss = B2.history['val_loss']
